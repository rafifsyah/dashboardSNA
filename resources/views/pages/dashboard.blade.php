                        ...
                        <div class="row">
                            <div class="col" style="max-width: 700px;">
                                ...
                                @if(!empty($commonValues))
                                    <h2>PENGGUNA INSTAGRAM</h2>
                                    <table class="numbered-table">
                                        <tr><th>NO</th><th>Pengguna Instagram</th></tr>
                                        @foreach ($commonValues as $index => $value)
                                            <tr><td>{{ $index + 1 }}</td><td>{{ htmlspecialchars($value) }}</td></tr>
                                        @endforeach
                                    </table>
                                    <p><a href="{{ url('storage/common_values.json') }}" download="common_values.json"><button>Download JSON File</button></a></p>
                                @else
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function runPythonScript(scriptName) {
            alert("This function needs to be implemented with script: " + scriptName);
        }
    });
</script>
@endpush
